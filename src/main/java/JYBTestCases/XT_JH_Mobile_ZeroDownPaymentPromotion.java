...
public class XT_JH_Mobile_ZeroDownPaymentPromotion {
	
	private static RunUnit_Service runService;
		
	@BeforeTest
	private void stup() throws Exception{
		Test_Unit testunit = TestXmlParse_Service.parse("src/main/java/JYBTestCaseXml/XT_JH_Mobile_ZeroDownPaymentPromotion.xml");
		runService = new RunUnit_Service(testunit);
		System.out.println("-----------------------------------【XT_JH模式下购买0首付促销产品的测试场景点】-----------------------------------");
	}
	...
	public void case_23() throws Exception{
		...
		runService.TestReportRemarks("验证在现行列界面,进入数据库根据合同号查询后,该合同属于XT_JH合作模式");
	}
