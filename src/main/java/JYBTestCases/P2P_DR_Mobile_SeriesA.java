...
public class P2P_DR_Mobile_SeriesA {
	
	private static RunUnit_Service runService;
		
	@BeforeTest
	private void stup() throws Exception{
		Test_Unit testunit = TestXmlParse_Service.parse1("src/main/java/JYBTestCaseXml/P2P_DR_Mobile_SeriesA.xml");
		runService = new RunUnit_Service(testunit);
		System.out.println("-----------------------------------【P2P_DR模式下购买手机系列A产品的测试场景点】-----------------------------------");
	}
	...
	public void case_26() throws Exception{
		...
		runService.TestReportRemarks("验证在现行列界面,进入数据库根据合同号查询后,该合同属于P2P_DR合作模式");
	}
