...
public class P2P_TBJ_Mobile_SeriesB {
	
	private static RunUnit_Service runService;
		
	@BeforeTest
	private static void stup() throws Exception{
		Test_Unit testunit = TestXmlParse_Service.parse("src/main/java/JYBTestCaseXml/P2P_TBJ_Mobile_SeriesB.xml");
		runService = new RunUnit_Service(testunit);
		System.out.println("-----------------------------------【P2P_TBJ模式下购买手机系列B产品的测试场景点】-----------------------------------");
	}
	...
	public void case_23() throws Exception{
		...
		runService.TestReportRemarks("验证在现行列界面,进入数据库根据合同号查询后,该合同属于P2P_TBJ合作模式");
	}
