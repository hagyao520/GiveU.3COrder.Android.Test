...
public class Appium_Util {
	 ...
	 * @throws Exception 
	 */
	public static WebElement getElement(Test_Step step) throws Exception{
...
//		   System.err.println(locatename+"  "+locatevalue+" "+locindex);
		   ...
		   switch(locatename){
		   		case "resource-id": 
		   			e = step.getDriver().findElements(By.id(locatevalue));
		   			break;
		   		case "text": 
		   			{
		   				e = step.getDriver().findElementsByAndroidUIAutomator("new UiSelector().text(\""+locatevalue+"\")");
		   			}
		   			break;
		   		case "class": 
		   			e = step.getDriver().findElements(By.className(locatevalue));
		   			break;
		   		case "content-desc": 
		   			e = step.getDriver().findElementsByAccessibilityId(locatevalue);
//		   			e = step.getDriver().findElementsByXPath("//android.view.View[@content-desc='业务说明']");
//		   			e = step.getDriver().findElementsByXPath("//android.view.View[@content-desc='"+locatevalue+"']");
		   			break;
		   		default:
		   			throw new Exception("step元素locator属性配置有误，'='之前必须为resource-id、text、calss和content-desc之一！");
		   }
